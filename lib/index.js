...
export default {
	...
	activate: function(state) {
		...
		// Check if minify
		this.minify = atom.config.get('Autoless.minify') || false;
		...
		this.disposables.add(atom.commands.add('atom-workspace', {
			'autoless:compile': () => {
				...
				if(!editor || !editor.getTitle().endsWith('.less')) {
					atom.notifications.addWarning(`Autoless: No '.less' file currently selected!`);
					return;
				}
		...
		if(compilePath.endsWith('.less')) {
			...
			if(path) {
				...
				if(config) {
					...
					try {
						execSync(`lessc ${path}/${config.SOURCE_FILE} ${path}/${config.TARGET_FILE}${this.minify ? ' --clean-css' : ''}`);
						atom.notifications.addSuccess(`Autoless: Successfully compiled '${config.SOURCE_FILE}'`);
					}
					catch(err) {
						let message = err.message;
						if(message.includes('ParseError')) {
							message = message.substr(message.indexOf('\n') + 1);
							message = message.replace(/[\u001b\u009b][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><]/g, '');
						}
						let stack = err.stack.replace(/[\u001b\u009b][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><]/g, '');
						atom.notifications.addError(`Autoless: Failed to compile '${config.SOURCE_FILE}' to '${config.TARGET_FILE}'!`, {
							detail: message,
							stack: stack,
							dismissable: true
						});
